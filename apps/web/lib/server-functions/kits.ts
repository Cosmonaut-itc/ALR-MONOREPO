...
export const fetchAllEmployeesServer = async (userId: string) => {
	if (!userId) {
	...
	const url = new URL('/api/auth/employee/all', origin);
	...
	if (!res.ok) {
		const text = await res.text().catch(() => '');
		throw new Error(`Employees fetch failed: ${res.status} ${res.statusText} ${text}`);
	}
	return res.json();
};
