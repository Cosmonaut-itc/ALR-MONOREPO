...
import { validateProductsResponse, type ApiResponse, type DataItemArticulosType, type ProductStockItem, type WithdrawOrderDetails } from "../types/types";
 ...
 */
export const getWithdrawalOrdersDetails = async (date: string): Promise<WithdrawOrderDetails[]> => {
    try {
        const response = await client.api["withdraw-orders"].details.$get({ query: { dateWithdraw: date } });
        if (!response.ok) {
            throw new Error(`API request failed with status: ${response.status}`);
        }
        const data: ApiResponse<WithdrawOrderDetails[]> = await response.json();
        return data.data || [];
    } catch (error) {
